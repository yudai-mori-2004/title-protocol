...
| §4.2 | Ticket (incremental reservation / release) | [x] | crates/tee/src/resource_pool.rs (Ticket: CAS-loop extend/shrink, RAII Drop auto-release, AtomicU64 last_activity_nanos timeout tracking → Send + Sync で Arc 共有可能, 14 tests incl. 2 concurrent) | 09, 19 |
| §4.3 | Memory pattern: single file (Range Request) | [x] | crates/core/src/content_stream.rs (ContentSource trait + InMemorySource + peak_memory_hint + `contract` test helper) + crates/tee/src/range_source.rs (HttpRangeSource via http-range-client 0.9, SafeRangeReader: get_bytes 直接叩き + file_size clip で末尾跨ぎバッファロスト防止 + HTTP 416→Ok(0) 正規化) + crates/tee/src/proxy_fetcher.rs (ProxyRangeSource via SyncBufferedHttpRangeClient<ProxyRangeBackend>, 同じ SafeRangeReader を共有) + crates/tee/src/content_fetch.rs (fetch_streaming → peak_memory_hint で漸進予約: in-memory=full size, range=reader buffer = `min_req_size` = 4 MiB). 50 GB streaming reservation test + 境界条件 contract suite (body_len 1..1025 × 末尾跨ぎ) で Read/Seek 全規約を構造的に検証。 | 09, 19 |
| §4.3 | Memory pattern: fragmented | [x] | crates/tee/src/fragmented_source.rs (FragmentedSource + FragmentedReader: init を Arc<[u8]> 常駐、fragments を lazy load で 1 個ずつ swap、peak_memory_hint は init + in-memory 常駐合計 + active Range バッファ最大、ContentSource::is_in_memory_resident() の明示 API で常駐判定) + crates/tee/src/content_fetch.rs (fetch_fragmented: 漸進予約モード = probe loop 内で各 fragment 取得直後に ticket.extend + 最終 peak_memory_hint まで shrink). 仕様 §4.3 「init + 1 fragment + Reader 内部状態」のピーク要件 + admission gate が loop 中の任意時点で正確な pool.used を見られる契約を達成。実 C2PA 署名付き JPEG を split して c2pa::Reader::with_stream 経由で parse 成功する e2e テスト 3 件 + ContentSource contract suite (境界条件) + Seek 規約準拠 (i64::MIN 含む) + Range fragment peak テスト + adversarial reject mid-loop テスト。 | 09, 19 |
...
| §4.4 | Chunk timeout (60s) | [x] | crates/tee/src/resource_pool.rs (Ticket::extend checks AtomicU64 last_activity_nanos against CHUNK_TIMEOUT) + crates/tee/src/limits.rs (CHUNK_TIMEOUT constant) | 09, 19 |
...
| §5.2 | Content fetch: single (HTTP GET + Range Request) | [x] | crates/tee/src/content_fetch.rs (HttpContentFetcher::fetch_streaming → HttpRangeSource w/ HEAD probe + fallback) + crates/tee/src/range_source.rs (HttpRangeSource via http-range-client, fake-server tests) + crates/tee/src/proxy_fetcher.rs (ProxyContentFetcher::fetch_streaming → ProxyRangeSource via wire-protocol HEAD/GET_RANGE) | 01, 04, 19 |
| §5.2 | Content fetch: fragmented | [x] | crates/tee/src/content_fetch.rs (fetch_fragmented: HEAD probe → FragmentedSource 構築 + 漸進予約) + crates/tee/src/fragmented_source.rs (FragmentedReader: init + lazy fragment swap で peak = init + 1 fragment + Range バッファ最大) + c2pa::Reader::with_stream 統合. | 01, 04, 19 |

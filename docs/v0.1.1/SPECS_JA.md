...
オンチェーンのGlobalConfig PDA + 全TeeNodeAccount PDAを一括取得する。追加のHTTPリクエストは不要——全てSolana RPCのみで完結する。
...
デコード済みデータはコンテンツの生バイナリとは独立にホストメモリ上に保持される。WASMは `read_content_chunk`（生バイナリ）と `read_decoded_chunk`（デコード済みデータ）の両方に同時にアクセスできる。これにより、例えばpHash計算（デコード済みピクセルが必要）と `hash_content`（生バイナリに対するハッシュ計算）を同一WASM実行内で組み合わせることが可能である。
...
暗号プリミティブホスト関数は `read_content_chunk` と同一のアドレッシングパターンを採用する。WASMは計算対象のデータを引数として渡すのではなく、TEEホストメモリ上のコンテンツに対するオフセットと長さのみを指定する。TEEホストは指定範囲のデータに対してネイティブ速度で暗号処理を実行し、結果をWASMリニアメモリの指定位置に書き込む。
...
WASM                                           TEE Host
  │                                               │
  │  n = hash_content(0, 0, content_len, out_buf) │
  │──────────────────────────────────────────────>│
  │                                               │  ホストメモリ上のコンテンツに対して
  │                                               │  ネイティブ速度でSHA-256を計算
  │<──────────────────────────────────────────────│
...
| 関数 | 引数 | 戻り値 | 説明 |
| --- | --- | --- | --- |
| `hash_content` | algorithm, offset, length, out_ptr | u32（出力バイト数、エラー時0） | 指定範囲のコンテンツに対するハッシュを計算し、`out_ptr` に書き込む |
| `hmac_content` | algorithm, key_ptr, key_len, offset, length, out_ptr | u32（出力バイト数、エラー時0） | 指定範囲のコンテンツに対するHMACを計算し、`out_ptr` に書き込む。鍵はWASMリニアメモリから読み取る |

`offset` と `length` は `read_content_chunk` と同一の座標系を使用する。すなわち、TEEホストが保持するコンテンツの生データ（C2PA署名付きコンテンツ、または署名なしコンテンツ＋オプションのサイドカー）に対するバイトオフセットと長さである。`out_ptr` はWASMリニアメモリ上の書き込み先バッファポインタである。

**サポートするアルゴリズム:**

アルゴリズムはu32の数値コードで指定する。

| algorithm（u32） | アルゴリズム | 出力サイズ |
| --- | --- | --- |
| `0` | SHA-256 | 32バイト |
| `1` | SHA-384 | 48バイト |
| `2` | SHA-512 | 64バイト |

サポートアルゴリズムの追加はTEEコードの更新に伴い行われる。WASMが未サポートのアルゴリズムを指定した場合、ホスト関数は0を返す。

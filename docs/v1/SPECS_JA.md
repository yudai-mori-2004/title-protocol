...
Global ConfigとTeeNodeAccount PDAから信頼されたTEEノードの一覧を取得する。全ての情報はオンチェーンから取得されるため、追加のHTTPリクエストは不要である。

```tsx
const nodes = await sdk.discoverNodes({
  status: 'Active',
});
// 1. GlobalConfig から trusted_node_keys を取得
// 2. 各 signing_pubkey に対応する TeeNodeAccount PDA を読み取る
// 3. status 等の条件でフィルタリング
// Returns: TeeNodeInfo[]
```

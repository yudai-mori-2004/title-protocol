...
echo ""
echo "Next steps (run from your local workstation):"
echo "  1. scp this directory back: scp -r '${OUT_DIR}/*' ./deploy/aws/build/registration/"
echo "  2. title-cli add-measurement --admin keys/admin.json --pcr0-hex 0x\$(jq -r .PCR0 measurements.json)"
echo "  3. bash deploy/aws/scripts/prover-run.sh                    # generates proof (~90 min)"
echo "  4. title-cli register-key --payer keys/admin.json --bundle deploy/aws/build/registration"

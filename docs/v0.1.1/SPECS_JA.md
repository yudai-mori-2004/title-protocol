...
## 0.2 C2PAとTitle Protocol

C2PA（Coalition for Content Provenance and Authenticity）は、Adobe、Microsoft、Intel等が策定したデジタルコンテンツの来歴証明規格である。コンテンツに署名付きメタデータ（マニフェスト）を埋め込み、「誰が、いつ、どのツールで作成・編集したか」を暗号学的に証明する。

C2PAが採用する「事前の記録と暗号署名」によるアプローチは、事後的なAI検出や電子透かしと異なり、生成技術の進歩に影響されない構造的な基盤を持つ。暗号署名の安全性は署名アルゴリズムとハッシュ関数の数学的性質に依存しており、コンテンツ生成技術がいくら高度化しても署名の安全性は損なわれない。Title Protocolは、C2PAのこの構造的優位性を前提として設計されている。

C2PAは来歴（Provenance）と同一性（Integrity）を解決する。Title Protocolは、帰属（Attribution）を解決する。
...
## 0.3 Title Protocolの役割: 検証済み属性をコンテンツから独立させる

C2PAはコンテンツの来歴と同一性を暗号学的に保証する。しかし、C2PAの検証済み属性（ハードウェア撮影証明、タイムスタンプ、編集履歴、素材の来歴等）はコンテンツのファイル内に閉じ込められている。

C2PAの完全性検証には、署名チェーンを格納するフルマニフェストと、コンテンツのオリジナルバイナリの両方が必要であり、完全性保証はバイト単位のハッシュ一致（C2PA仕様のhard binding）に依拠する。しかし、現状ほぼ全てのSNS・メッセージアプリ・CDNはアップロード時にマニフェストを削除しコンテンツを再圧縮する。コンテンツが流通する主要な経路で、検証済み属性へのアクセスは失われる。

属性がファイルに閉じ込められていることは、さらに二つの問題を引き起こす。

**権利帰属がサービスを跨げない。** C2PAはコンテンツの識別子を持つが、「現在誰が権利を持っているか」は記録しない。既存の権利管理の仕組みは各サービスのデータベースに閉じており、あるサービスでの権利登録が別のサービスで認識されることはない。出自の証明と権利者の情報が不可分に結びつき、サービスを横断して参照可能な形で記録されなければ、コンテンツの権利者をプログラマティックに特定することはできない。

**検証結果の取得にサーバーの信頼が必要になる。** サーバーに属性を保存する方式では、サーバー側が情報を改ざんしないことを信じるしかない。報道・司法・保険など中立性が求められる領域では、情報の提供者自体が信頼できるかという問題が避けられない。

これら三つの問題（メタデータの消失、権利帰属のサービス閉鎖性、取得経路における信頼）の根本原因は共通している。**検証済み属性が、コンテンツやサービスやサーバーといった特定の場所に閉じ込められている**ことである。

Title Protocolは、この根本原因を以下の三つのステップで解決する。

1. **抽出**: TEE（Trusted Execution Environment: 信頼された実行環境）内でC2PAコンテンツから属性を検証・抽出し、TEE自身の秘密鍵で署名する。TEEの内部処理はノード運営者を含む誰も改ざん・閲覧できない
2. **記録**: 署名済みの属性をブロックチェーン上に永続化する。コンテンツの生データが失われた後も、属性は独立した記録として存続する
3. **帰属**: 記録をウォレットアドレスに紐付ける。権利トークン（cNFT）の保持者が権利者として解決され、トークンの移転がそのまま権利の譲渡を意味する

この構造により、検証済み属性はコンテンツから独立する。オリジナルファイルがなくても、特定のサービスに依存しなくても、特定のサーバーを信頼しなくても、コンテンツの属性と権利者に誰でもアクセスでき、その正当性を独立に検証できる。属性の抽出・記録・帰属という汎用的な構造の上に、あらゆるアプリケーションが自由にロジックを構築できる状態が生まれる。
...
| Smart-Contract-Less | 独自のMintロジックを持たず、Metaplex標準（Bubblegum cNFT + MPL Coreコレクション）の機能をそのまま活用する |
...
| **身分証明書** | **SSL/TLSサーバー証明書**（X.509規格）— 通信先が意図したドメイン（サーバー）であることを証明する | **C2PAマニフェスト**（C2PA規格）— データの来歴に改ざんがなく、同一性の担保されたコンテンツであることを証明する |
| **監査ログ** | **Certificate Transparency (CT) ログ** — 証明書の発行履歴を監視する公開台帳 | **Title Protocol (Core Layer)** — TEE検証結果を刻むブロックチェーン台帳 |
| **宛先解決** | **DNS (Domain Name System)** — ドメイン名からIPアドレス（場所）を解決 | **Title Protocol (Resolve)** — コンテンツに記録されたC2PAのデータに基づく識別子からブロックチェーンウォレット（権利者のSolanaウォレットアドレス）を解決 |
...
| signed_json（署名済みJSON） | TEEが検証結果をJSON形式でまとめ、自身の秘密鍵で署名したデータオブジェクト。オフチェーンストレージに保存された状態を指す場合は「オフチェーンデータ」とも称する |
...
### 二段階の処理: TEEを純粋な計算機に留める

TEEがコンテンツの復号・検証に成功した後、そのままオフチェーンストレージへの保存と権利トークンの発行まで一気に完結できれば最も単純だが、これには問題がある。

TEEが外部ストレージに直接書き込むためには、認証情報（APIキーやトークン）の保持、ストレージプロバイダのAPI仕様への結合、ネットワーク障害時のリトライロジックなど、検証とは無関係な複雑さをTEE内部に持ち込むことになる。これはストレージが無料であっても有料であっても同様であり、入力データに基づいて検証し署名した結果を返すだけの純粋な計算機というTEEの設計原則を損なう。さらに、クライアントがストレージプロバイダを自由に選択する余地も失われる。

加えて、権利トークンのメタデータにはオフチェーンデータへのURIが含まれるため、トランザクションを構築する前にオフチェーンデータの永続化とURIの確定が完了している必要があるという順序制約が存在する。
...
> **注記:** `signed_json` に時刻による有効期限は設けない。Phase 2における `tee_signature` の検証は、それを生成したTEEインスタンスの鍵でのみ成功するため、Phase 1完了後にTEEが再起動され鍵がローテーションされた場合、まだPhase 2を通過していない `signed_json` は署名検証に失敗し拒否される。一方、既にPhase 2を通過しcNFTとして発行された記録は、ブロックチェーン上に永続化されており鍵のローテーションの影響を受けない。
> 
> 
> この設計の主たるセキュリティ効果は、TEEの侵害時における被害の時間的局所化である。同一のTEEインスタンスが永続的に稼働し続けることがないため、仮にサイドチャネル攻撃等によりTEEが侵害された場合でも、その影響は当該インスタンスの鍵のライフサイクル内で処理されたコンテンツに限定され、プロトコル全体の過去の記録に遡及的に波及することはない。副次的に、鍵のライフサイクルが未使用の `signed_json` の有効期間となるため、信頼できないホスト時刻への依存も排除される。
> 
...
1.1で述べた「二値的な存在」、すなわち「存在し検証可能」か「削除されて存在しない」かの二択は、この署名検証によって実現される。
...
| --- | --- |
| **検証対象** | **C2PA署名付きコンテンツの生データ** — コンテンツの生データに基づき、埋め込まれた署名チェーンとマニフェストを検証する。 |
| **識別子** | **アクティブマニフェスト署名のSHA256ハッシュ** — コンテンツの現在の状態に対応するC2PA署名のハッシュ値を、コンテンツの現在の状態に対する一意なIDとして使用する。 |
| **記録される事実** | **来歴グラフ（DAG）** — 過去の全来歴、すなわち素材や過去の状態に対応する全てのマニフェスト署名を、有向非巡回グラフ構造で記録する。 |
...
### Extensionの動機: C2PAメタデータ消失への対応

Coreのcontent_hashはC2PAのアクティブマニフェスト署名から決定論的に算出される。しかし、§0.3で述べた通り、SNS上で流通する過程でC2PAメタデータが削除されたコンテンツからはcontent_hashを取得できない。Coreのみのプロトコルでは、メタデータが失われた時点でコンテンツとプロトコル上の記録を結びつける手段が消滅する。

この制約に対し、Extensionはコンテンツの実体そのものから算出される属性（知覚ハッシュ、ハードウェア撮影証明、ライセンスフラグ等）をブロックチェーンに記録する。例えば、SNS上のサムネイルから知覚ハッシュを算出し、プロトコル上の登録済みコンテンツを逆引きすることで、C2PAメタデータに依存しない検索経路が確保される。

CoreとExtensionの二層分離は、この「メタデータなしでもコンテンツの証明にアクセスできる経路」を確保するための設計判断である。
...
| --- | --- |
| **検証対象** | **C2PA署名付きコンテンツの生データ + オプションの補助入力（extension_inputs）** — コンテンツの生データと、クライアントが提供する補助入力（外部計算結果やその証明等）を入力として、決定論的なWASMモジュールを実行する。補助入力が不要なWASMはコンテンツの生データのみで動作する。 |
| **識別子** | **Coreの識別子 + Extension ID** — Coreで定義された識別子（マニフェスト署名ハッシュ）と、適用したWASMのIDの組み合わせを一意なIDとして使用する。 |
| **記録される事実** | **コンテンツデータの客観的属性** — WASMの実行結果として算出された値（知覚ハッシュ、特徴量、フラグ等）を記録する。補助入力を使用した場合は、その入力のハッシュも併せて記録され、第三者による再現検証を可能にする。 |
...
Title Protocolの役割は「誰に払うべきか」を返すことだけである。分配比率、手数料、ロイヤリティの計算はマーケットプレイスが自身のビジネスロジックとして決定する。
...
報道機関、裁判所、保険会社が「この写真は実際にカメラで撮影されたものか」という問いに対して、第三者が独立に検証可能な回答を提供できるようになる。
...
「間違えてBurnした場合、誰かに乗っ取られるのでは」という懸念に対しては、乗っ取りには対象コンテンツのC2PA付き元データが必要であり、正規のTEE検証プロセスを通過しなければならない。元データが流出していない限り、即座の乗っ取りは困難である。
...
### アプリケーションとエンドユーザー間の信頼境界

プロトコルの信頼モデルは、ペイロードを構築・暗号化したクライアントまでを保護範囲とする。E2EEにより、ノード運営者を含む全ての中間者はペイロード内の帰属先ウォレットを改ざんも閲覧もできない。

しかし、クライアント（SDK）を呼び出すアプリケーションが、エンドユーザーのウォレットではなく自身のウォレットを帰属先として設定することは、プロトコルの視点からは正常なリクエストと区別できない。TEEに到達するのは暗号化済みペイロードであり、その構築過程における「誰の意思で、誰のウォレットが指定されたか」はTEEの視界の外にある。

これは原理的な制約である。仮にペイロード構築時にウォレット署名を必須としても、悪意あるアプリケーションはユーザーのコンテンツをサーバーに転送し、自身のウォレットで全フローを実行できる。プロトコル層でこの攻撃を防ぐことは、暗号学的に不可能である。

したがって、プロトコルは以下の境界を明確にする。

| 責任範囲 | 担当 |
| --- | --- |
| ペイロードの秘匿性と完全性（クライアント〜TEE間） | プロトコル |
| アプリケーションがエンドユーザーの帰属を正しく扱っていること | アプリケーション開発者 |

アプリケーション開発者がエンドユーザーからの信頼を獲得するための手段としては、クライアントコードのオープンソース化、第三者による監査、再現可能ビルドの提供などが考えられる。これらはプロトコルが強制するものではなく、アプリケーション開発者が自らの信頼性を社会的に証明するための手段である。

この構造は、Webにおける TLS とサイト運営者の関係と同型である。TLS は通信経路の安全性を保証するが、サイト運営者が善良であるかどうかはブランド・評判・監査によって判断される。同様に、Title Protocol はコンテンツの帰属記録の完全性を保証するが、アプリケーションがエンドユーザーの利益に沿って動作しているかどうかは、アプリケーション層の透明性に委ねられる。
...
検証の起点となるブロックチェーン上のアカウント群。Solana上のPDA（Program Derived Address）として存在する。
...
├── authority: Pubkey                    // DAO multi-sig
├── core_collection_mint: Pubkey         // Core cNFTの公式コレクション
...
├── signing_pubkey: [u8; 32]             // Ed25519署名用公開鍵
├── encryption_pubkey: [u8; 32]          // X25519暗号化用公開鍵
├── gateway_pubkey: [u8; 32]             // Gateway署名用Ed25519公開鍵
├── gateway_endpoint: String             // GatewayエンドポイントURL
├── status: u8                           // 0=Inactive, 1=Active
├── tee_type: u8                         // 0=aws_nitro, 1=amd_sev_snp, 2=intel_tdx
├── measurements: Vec<MeasurementEntry>  // 期待される測定値
│   └── { key: [u8; 16], value: [u8; 48] }
...
検証ツールやSDKは、上記の2種類のアカウントを集約して以下の論理ビューを構築する。
...
```json
{
  ...
  "trusted_tsa_keys": [
    "0xプレフィックス付きhexエンコードされたTSA公開鍵のSHA-256ハッシュ（32バイト）"
  ],
...
| フィールド | 用途 |
| --- | --- |
| `signing_pubkey` | TEE署名用公開鍵。cNFT発行トランザクションの部分署名、signed_jsonの署名検証に使用 |
| `encryption_pubkey` | クライアントがE2EEペイロードを暗号化する際のX25519公開鍵 |
| `gateway_pubkey` | Gatewayの署名検証に使用する公開鍵。TEEはこの鍵で署名されたリクエストのみを受け付ける |
| `gateway_endpoint` | ノードのGateway URLエンドポイント |
| `tee_type` | TEEの種別。Attestation Documentの検証手順と `expected_measurements` の解釈は本フィールドに依存する |
| `expected_measurements` | TEEインスタンスの期待される測定値。Attestation Documentから抽出した測定値と照合することで、TEEが正しいコードを実行していたことを事後的に検証できる。内部構造は `tee_type` ごとに異なる |
...
クライアント（SDK）がノードを選択するために必要な情報は、全てオンチェーン（GlobalConfigAccount + TeeNodeAccount PDA）で管理される。SDKはSolana RPCからこれらのアカウントを読み取ることで、HTTPラウンドトリップなしにノード一覧とスペック情報を取得できる。

対応するExtensionの一覧は、GlobalConfigの `trusted_wasm_modules` から決定論的に導出される（全てのアクティブノードは全ての信頼されたWASMを実行可能である）。

Gatewayは課金ティアごとに異なるリソース制限を適用する場合、認証済みリクエストに対してティア別の制限情報を返却するエンドポイントを別途提供できる。具体的なエンドポイント設計はノード運営者に委ねる。
...
TEEは4組のキーペアをTEE内部（メモリ上）でのみ生成・保持する。
...
TEEは起動直後、`inactive` 状態で `/create-tree` エンドポイントを一度だけ公開する。Core用とExtension用の2つのMerkle Treeを同時に作成する。
...
Response:
{
  "core_signed_tx": "Base64エンコードされたCore Tree完全署名済みトランザクション",
  "core_tree_address": "Base58エンコードされたCore Merkle Treeアドレス",
  "ext_signed_tx": "Base64エンコードされたExtension Tree完全署名済みトランザクション",
  "ext_tree_address": "Base58エンコードされたExtension Merkle Treeアドレス",
  "signing_pubkey": "Base58エンコードされたEd25519公開鍵（署名用）",
  "encryption_pubkey": "Base64エンコードされたX25519公開鍵（暗号化用）"
}
```

TEEの署名用キーペア（payer兼tree_creator）がfee payerとなるため、各トランザクションはTEE内部で完全署名される。payerをTEE内部walletにすることで、Merkle Treeの作成・操作権限が完全にTEE内部に閉じる。ノード運営者は返却されたトランザクションをそのままSolanaにブロードキャストする。
...
TEE起動
  │
  ▼
[inactive] ── /create-tree ──> signed_tx返却（Core + Extension の2トランザクション）
  │
  ▼
ノード運営者: ブロードキャスト（2トランザクション）
  │
  ▼
...
TEEノードのオンチェーン登録用エンドポイント。TEE内部で `register_tee_node` Anchor命令のトランザクションを構築し、TEEの署名鍵で部分署名して返す。DAO authorityの共同署名後にブロードキャスト可能となる。
...
TEEのsigning_pubkeyがfee payerとなり、鍵所有が暗号的に証明される。ノード運営者はDAOの authority に共同署名を依頼し、完全署名されたトランザクションをブロードキャストする。

---

### /health エンドポイント

TEEサーバーのヘルスチェック用エンドポイント。
...
| `c2pa_max_graph_size` | 10000 | C2PAマニフェストのグラフの読み込み可能な最大サイズ。ノード+エッジの最大値であり、計算中にこの最大サイズを超えるとエラーを返す |
...
### fetchGlobalConfig(cluster)

オンチェーンのGlobalConfig PDA + 全TeeNodeAccount PDAを一括取得する。追加のHTTPリクエストは不要であり、全てSolana RPCのみで完結する。

```tsx
// シンプル（デフォルトRPC）
const config = await fetchGlobalConfig("devnet");

// カスタムRPC（Helius等の有料RPC）
const conn = new Connection("https://devnet.helius-rpc.com/?api-key=...");
const config = await fetchGlobalConfig(conn, "devnet");
```

戻り値の `GlobalConfig` には以下が含まれる:
...
`fetchGlobalConfig` の戻り値をそのまま渡す。ノードのgateway\_endpointは `GlobalConfig.trusted_tee_nodes` から自動取得される。

### client.register(options)

コンテンツの検証・メタデータ保存・cNFT発行を一括実行する。E2EE暗号化、Temporary Storageアップロード、セキュリティ検証を全て内部で処理する。
...
| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| `content` | `Uint8Array` | Yes | コンテンツのバイナリデータ |
| `ownerWallet` | `string` | Yes | 所有者ウォレットアドレス（Base58） |
| `processorIds` | `string[]` | No | 実行するプロセッサID（デフォルト: `['core-c2pa']`） |
| `extensionInputs` | `Record<string, unknown>` | No | 補助入力が必要なExtensionのみ指定。キーはextension\_id。 |
| `storeSignedJson` | `(json: string) => Promise<string>` | Yes | signed\_jsonの永続化コールバック。URIを返す。 |
| `node` | `TeeSession` | No | 使用するノード。省略時はヘルスチェック付きで自動選択。 |
| `delegateMint` | `boolean` | No | `true`: Gateway経由でブロードキャストまで実行。デフォルト `false`。 |
| `recentBlockhash` | `string` | No | Solana blockhash。省略時は `delegateMint: false` では不要（`/sign` で使用）。 |
...
1. ノード選択（未指定の場合、ヘルスチェック付きで自動選択）
2. エフェメラルX25519キーペア生成
3. ペイロード暗号化（E2EE: ECDH + HKDF-SHA256 + AES-256-GCM）
4. 暗号化ペイロードをTemporary Storageにアップロード
5. `/verify` 呼び出し
6. 暗号化レスポンスをエフェメラル秘密鍵で復号
7. **wasm\_hash検証**: signed\_jsonの `wasm_hash` をGlobalConfigの `trusted_wasm_modules` と照合（セキュリティクリティカル。後述）
8. `storeSignedJson` コールバックでsigned\_jsonを永続化 → URI取得
9. `/sign` または `/sign-and-mint` 呼び出し

**戻り値:**

```tsx
// delegateMint: false（デフォルト）
{
  contents: Array<{
    processorId: string,
    contentHash: string,
  ...
  partialTxs: string[]    // Base64。クライアントがウォレットで共同署名しブロードキャストする。
}

// delegateMint: true
{
  contents: Array<{ ... }>,
  txSignatures: string[]  // Gateway経由でブロードキャスト済み
}
```

### resolve(contentHash)

content\_hashに対応するcNFTと来歴グラフを解決する。DAS API（Helius等）またはIndexerとの連携が必要であり、将来バージョンで実装予定。

### SDKによる自動検証

SDKは `register()` 内で以下の検証を自動実行する。これらはプロトコルの安全性を高める推奨プラクティスであり、SDKが利便性のために内蔵する。SDKを使わない場合でも、クライアントはオンチェーンのGlobalConfigを直接参照して同等の検証を自ら実行できる。
...
1. トランザクションに含まれるcontent\_hashが、自分がアップロードしたコンテンツのハッシュと一致するか
2. 署名者がGlobalConfigの信頼リストに含まれるか
...
WASMモジュールはTEE内のサンドボックス（wasmtime）で実行される。
...
WASM                                     TEE Host
  ...
  │         コンテンツの総バイト数（u32）       │
  │                                         │
  │  n = read_content_chunk(0, 64KB, buf)   │
  │────────────────────────────────────────>│  ホストがWASMリニアメモリの
  │<────────────────────────────────────────│  buf位置にデータを書き込み
  │         nバイトコピー済み                  │
  │                                         │
  │  n = read_content_chunk(64KB, 64KB, buf)│
  │────────────────────────────────────────>│
  │<────────────────────────────────────────│
  │         nバイトコピー済み                  │
  │                                         │
...
全てのデータ交換はWASMリニアメモリ上のバッファポインタ（u32）を介して行われる。ホスト関数はwasmtimeのLinker APIを通じてWASMモジュールに提供される。
...
WASM                                     TEE Host
  │                                         │
  │  size = get_extension_input(buf, len)   │
  │────────────────────────────────────────>│
  │<────────────────────────────────────────│
  │    size: 補助入力の実サイズ（u32）         │
  │    (buf にmin(size, len)バイトコピー済み)  │
  │                                         │
```

WASMは最初にバッファサイズ0で呼び出して実サイズを取得し、十分なバッファを確保した上で再度呼び出すことができる。補助入力が存在しない場合（内部完結型WASMの場合）、戻り値は0となる。

### ホスト側コンテンツデコード

画像・音声等のメディアコンテンツを処理するWASMでは、デコード処理（JPEG展開、PNG展開等）が必要となる。WASMモジュール内でデコーダをリンクする方式には以下の課題がある：

- **バイナリ肥大化**: フォーマットごとのデコーダを含めるとWASMサイズが増大
- **Fuel消費**: ソフトウェアデコードにFuel予算の大部分を消費
...
WASM                                           TEE Host
  ...
  │                                               │
  │  len = get_decoded_length()                   │
  │──────────────────────────────────────────────>│
  │<──────────────────────────────────────────────│
  │    len: デコード済みデータの総バイト数             │
  │                                               │
  │  n = read_decoded_chunk(offset, length, buf)  │
  │──────────────────────────────────────────────>│
  │<──────────────────────────────────────────────│
...
WASMがコンテンツやデコード済みデータに対して特徴量計算（ハッシュ、画像処理等）を行う場合、データ全体をWASMメモリに転送して処理するのは非効率である。特にデコード済みピクセルデータは数MBに達し、WASM側でのgrayscale変換やリサイズはfuel消費の大部分を占める。

これを解決するため、TEEホストは `get_content_feature` / `get_decoded_feature` ホスト関数を提供する。WASMはJSON specで「何を計算してほしいか」を指定し、ホストがネイティブ速度で実行、小さな結果だけWASMメモリに返す。

**設計原則:**

- JSON specで計算内容を指定（ABI変更なしで新しいopを追加可能）
- 対称的なシグネチャ: `(spec_ptr, spec_len, output_ptr) -> i32`
- 戻り値: 出力バイト数（正値）またはエラーコード（負値）

```
WASM                                                TEE Host
  │                                                    │
  │  n = get_content_feature(spec, len, out_buf)       │
  │  spec = {"op":"sha256"}                            │
  │───────────────────────────────────────────────────>│
  │                                                    │  ホストメモリ上のコンテンツに対して
  │                                                    │  ネイティブ速度でSHA-256を計算
...
全てのホスト関数は `"env"` モジュール名でwasmtimeのLinkerに登録される。データ交換はWASMリニアメモリ上のバッファポインタ（u32）を介して行われる。

| 関数名 | シグネチャ | 説明 |
| --- | --- | --- |
| `get_content_length` | `() -> u32` | コンテンツの総バイト数を返す |
...
WASMモジュールは以下のエクスポート関数を提供する。

| 関数名 | シグネチャ | 説明 |
| --- | --- | --- |
| `process` | `() -> u32` | メインエントリポイント。ホスト関数を通じてコンテンツと補助入力にアクセスし、結果バッファへのポインタを返す |
| `alloc` | `(size: u32) -> u32` | WASMリニアメモリ上にバッファを確保し、ポインタを返す。ホスト関数が結果の書き込みに使用 |

**結果バッファフォーマット:**

`process` が返すポインタは、WASMリニアメモリ上の以下のフォーマットのバッファを指す。

```
[4バイト LE: JSONの長さ][JSONバイト列...]
```

TEEホストはこのバッファからJSONを読み取り、Extension payloadに組み込む。
...
※ インフラコストはAWS Nitro Enclaves + S3 + EC2 (Gateway)の概算
